"""
train_sage.py — GraphSAGE baseline for analog circuit regression.
Trains on the same graph dataset as NEXUS-Lite, but without equilibrium iterations.
"""
...
def train():
    ...
    print(f"Parameters: {total:,}  (GraphSAGE baseline)")
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        if val_loss < best_val_loss:
            ...
            }, os.path.join(CHECKPOINT_DIR, "sage_model.pt"))
        else:
            patience_count += 1
    ...
    print(f"Checkpoint    → {CHECKPOINT_DIR}/sage_model.pt")
