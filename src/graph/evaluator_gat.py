"""
evaluator_gat.py — GAT baseline evaluation
Loads checkpoints/gat_model.pt, runs predictions on held-out val set,
computes MAE / RMSE / R² vs Ngspice, and saves scatter plots.

Run: python src/graph/evaluator_gat.py
Outputs:
  figures/gat_gain_scatter.png
  figures/gat_bw_scatter.png
  figures/gat_loss_curve.png
  data/graphs/eval_results_gat.json
"""
...
CHECKPOINT = "checkpoints/gat_model.pt"
LOG_PATH = "data/graphs/training_log_gat.json"
EVAL_OUT = "data/graphs/eval_results_gat.json"
FIGURES_DIR = "figures"
TRAIN_SPLIT = 0.8
SEED = 42
...
def main():
    ...
    try:
        ...
        ax.set_ylabel("GAT Predicted Gain (dB)")
        ax.set_title(f"Gain Prediction | MAE={mae_gain:.2f} dB  R²={r2_gain:.3f}")
        ax.grid(True, alpha=0.3)
        plt.tight_layout()
        plt.savefig(os.path.join(FIGURES_DIR, "gat_gain_scatter.png"), dpi=150)
        plt.close()
        print(f"Saved → {FIGURES_DIR}/gat_gain_scatter.png")
        ...
        ax.set_ylabel("GAT Predicted Bandwidth (MHz)")
        ax.set_title(f"Bandwidth Prediction | MAE={mae_bw:.1f} MHz  R²={r2_bw:.3f}")
        ax.grid(True, alpha=0.3, which="both")
        plt.tight_layout()
        plt.savefig(os.path.join(FIGURES_DIR, "gat_bw_scatter.png"), dpi=150)
        plt.close()
        print(f"Saved → {FIGURES_DIR}/gat_bw_scatter.png")
        ...
        ax.grid(True, alpha=0.3)
        plt.tight_layout()
        plt.savefig(os.path.join(FIGURES_DIR, "gat_loss_curve.png"), dpi=150)
        plt.close()
        print(f"Saved → {FIGURES_DIR}/gat_loss_curve.png")
